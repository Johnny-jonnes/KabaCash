...
import { DBAccount, DBActivityLog, DBBudget, DBBudgetPeriod, DBCategory, DBNotification, DBPlannedEntry, DBRecurringTransaction, DBSavingsGoal, DBSpace, DBSpaceInviteLevel, DBSpaceMember, DBSyncQueueItem, DBTransaction, DBTransactionTemplate } from '@/types/database';
...
export class KabaCashDB extends Dexie {
  ...
  spaceInviteLevels!: Table<DBSpaceInviteLevel>;
  ...
  constructor() {
    ...
    // Version 9 : niveaux d'accès par code d'invitation (chaque code = un jeu de
    // permissions prédéfini, appliqué automatiquement au membre qui rejoint avec ce code)
    this.version(9).stores({
      ...
      spaceMembers: 'id, space_id, user_id, role, sync_status, deleted_at',
      plannedEntries: 'id, user_id, space_id, planned_date, status, sync_status, deleted_at',
      spaceInviteLevels: 'id, space_id, invite_code, sync_status, deleted_at',
    });
  }
}
